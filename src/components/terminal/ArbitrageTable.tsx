...
import type { RegistrarPricing } from "@/services/mock";
import {
      ...
      cell: ({ row }) => {
        ...
        return (
          <span className={cn("font-mono text-xs", isCheapest ? "text-emerald-400 font-semibold" : "text-zinc-300")}>
            {val === 0 ? <span className="text-zinc-600">—</span> : `$${val.toFixed(2)}`}
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="font-mono text-xs text-zinc-400">
          {row.original.transfer === 0 ? <span className="text-zinc-600">—</span> : `$${row.original.transfer.toFixed(2)}`}
        </span>
      ),
    },
    {
